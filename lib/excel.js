...
async function buildWorkbook(transactions, fileStats = []) {
  ...
  // Sources sheet — diagnostic showing what was actually parsed from each input file.
  sources.columns = [
    { header: "File", key: "file", width: 50 },
    { header: "Detected Source", key: "source", width: 20 },
    { header: "Rows Parsed", key: "rows", width: 14 },
    { header: "Note", key: "note", width: 60 },
  ];
  sources.getRow(1).font = { bold: true, color: { argb: "FFFFFFFF" } };
  sources.getRow(1).fill = { type: "pattern", pattern: "solid", fgColor: { argb: "FF1F2A48" } };
  for (const fs of fileStats) {
    sources.addRow({ file: fs.file, source: fs.source, rows: fs.rows, note: fs.note || "" });
  }
