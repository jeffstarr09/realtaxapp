...
function rowsToTransactions(rows, source, filename) {
  ...
  const isAmazon =
    ...
    (hasAny("order id", "order id (oid)") &&
      ...
      hasAny("item total", "total owed", "total charged", "total amount", "item subtotal", "shipment item subtotal", "total"));
...
function parseAmazonRows(rows, filename) {
  ...
  for (const row of rows) {
    ...
    const qty = parseInt(pickCI(row, "Quantity", "Original Quantity") || "1", 10) || 1;
    const total =
      toNumber(pickCI(row, "Total Amount")) ??
      toNumber(pickCI(row, "Item Total")) ??
      toNumber(pickCI(row, "Total Owed")) ??
      toNumber(pickCI(row, "Total Charged")) ??
      toNumber(pickCI(row, "Shipment Item Subtotal")) ??
