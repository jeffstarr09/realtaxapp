...
function parseCsv(buffer, source, filename) {
  ...
  // Detect Amazon order-history CSV (from Amazon's "Request My Data" export).
  // Two known formats: legacy "Order Date / Title / Item Total" and the newer
  // Retail.OrderHistory.csv with columns "Order Date / Product Name / Total Owed".
  ...
  const isAmazon =
    ...
    hset.has("asin/isbn") ||
    hset.has("asin");
  if (isAmazon) return parseAmazonRows(res.data, filename);
